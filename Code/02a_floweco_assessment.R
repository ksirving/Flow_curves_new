...
delta_long <- delta.RB9  %>%    ### this line edited but left variable name the same for rest of code 
  ...
  filter(FlowMetric %in% c("ds_mag_50", "ds_mag_90", "fa_mag", "sp_mag", "wet_bfl_mag_10", "wet_bfl_mag_50", "q99")) %>%  #edited, put all mag variables here 
...
unique(delta_long$FlowMetric) ## 7 mag metrics
names(delta_long)
...
write.csv(delta_med, "output_data/02a_delta_med.csv") #edited by rachel 
...
write.csv(limits, "output_data/02a_limits.csv")
...
write.csv(delta_dfx, "ignore/02a_alteration_by_year_comid_all_sites.csv") 
